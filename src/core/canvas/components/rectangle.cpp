...
Rectangle::Rectangle(glm::vec2 pos, glm::vec2 size) : mRect{pos, size} { mBackgroundColor = {1.0f, 0.0f, 0.0f, 1.0f}; }

std::vector<Property> Rectangle::get_properties() {
    return {
        {.isOnSameLine = false, .type = PropertyType::Color, .name = "Background Color", .value = &mBackgroundColor},
        {.isOnSameLine = false, .type = PropertyType::Float, .name = "X Position", .value = &mRect.position.x, .step = 0.1f},
        {.isOnSameLine = true, .type = PropertyType::Float, .name = "Y Position", .value = &mRect.position.y, .step = 0.1f},
        {.isOnSameLine = false, .type = PropertyType::Float, .name = "Width", .value = &mRect.size.x, .step = 0.1f},
        {.isOnSameLine = true, .type = PropertyType::Float, .name = "Height", .value = &mRect.size.y, .step = 0.1f}
      };
}

void Rectangle::set_rect(Rect rect) { mRect = rect; }
Rect Rectangle::get_rect() const { return mRect; }

void Rectangle::set_position(glm::vec2 position) { mRect.position = position; }

void Rectangle::set_background_color(glm::vec4 color) { mBackgroundColor = color; }

glm::vec2 &Rectangle::get_position() { return mRect.position; }
