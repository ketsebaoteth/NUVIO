...
void ShortcutManager::process() {
    ...
    for (auto &_shortcut : mShortcuts) {
        bool already_handled = (std::find(handledShortcuts.begin(), handledShortcuts.end(), _shortcut) != handledShortcuts.end());
        ...
        if (_shortcut.modifiers.empty()) {
            bool noModifiersPressed =
                glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_LEFT_CONTROL) != GLFW_PRESS &&
    ...
    handledShortcuts.erase(
        std::remove_if(handledShortcuts.begin(), handledShortcuts.end(),
                       [](const shortcut &sc) {
                           int state =
                               (sc.type == ShortcutType::Key)
...
void ShortcutManager::unregister_shortcut(shortcut _shortcut) {
    mShortcuts.erase(
        std::remove(mShortcuts.begin(), mShortcuts.end(), _shortcut),
        mShortcuts.end());
}
